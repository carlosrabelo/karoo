...
func TestCopyID(t *testing.T) {
	tests := []struct {
		...
		{name: "nil id", id: nil, want: ""},
		{name: "number id", id: NewIntID(42), want: "42"},
		{name: "string id", id: json.RawMessage(`"abc"`), want: `"abc"`},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.want == "" {
				if got != nil {
					t.Errorf("CopyID() = %s, want nil", got)
				}
				return
			}
			if string(got) != tt.want {
				t.Errorf("CopyID() = %s, want %s", got, tt.want)
			}
			if len(tt.id) > 0 && &got[0] == &tt.id[0] {
				t.Error("CopyID() returned same backing array")
			}
		})
	}
}

func TestParseIDInt64(t *testing.T) {
	tests := []struct {
		name string
		id   json.RawMessage
		want int64
		...
		{name: "number", id: json.RawMessage(`7`), want: 7, ok: true},
		{name: "numeric string", id: json.RawMessage(`"9"`), want: 9, ok: true},
		{name: "non-numeric string", id: json.RawMessage(`"abc"`), ok: false},
		{name: "null", id: json.RawMessage(`null`), ok: false},
		{name: "empty", id: nil, ok: false},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, ok := ParseIDInt64(tt.id)
			if ok != tt.ok {
...
func TestMessageClassification(t *testing.T) {
	...
	// String IDs are valid request identifiers
	strReq := Message{ID: json.RawMessage(`"req-1"`), Method: MethodAuthorize}
	if !strReq.IsRequest() {
		t.Error("string id request should be classified as request")
	}
}
