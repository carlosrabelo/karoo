...
// ID accepts both JSON numbers and strings (common across miners/pools).
type Message struct {
...
// IDPresent reports whether id is a non-null Stratum identifier.
func IDPresent(id json.RawMessage) bool {
	return len(id) > 0 && string(id) != "null"
}

// CopyID creates a deep copy of a Stratum ID payload.
func CopyID(id json.RawMessage) json.RawMessage {
	if !IDPresent(id) {
		return nil
	}
	dup := make(json.RawMessage, len(id))
	copy(dup, id)
	return dup
}

// NewIntID encodes n as a JSON number ID.
func NewIntID(n int64) json.RawMessage {
	return json.RawMessage(strconv.FormatInt(n, 10))
}

// ParseIDInt64 extracts a numeric ID (number or numeric string).
func ParseIDInt64(id json.RawMessage) (int64, bool) {
	if !IDPresent(id) {
		return 0, false
	}
	var n int64
...
func NewErrorResponse(id json.RawMessage, code int, message string, details interface{}) Message {
	return Message{
		ID:    CopyID(id),
...
func NewSuccessResponse(id json.RawMessage, result interface{}) Message {
	return Message{
		ID:     CopyID(id),
		Result: result,
	}
...
func (m *Message) IsResponse() bool {
	return IDPresent(m.ID) && (m.Result != nil || m.Error != nil)
}
