...
// Target share rate is 1 share every TargetSeconds (independent of difficulty).
func (m *Manager) calculateNewDifficulty(stats *ClientStats) float64 {
	if m.cfg.TargetSeconds <= 0 {
		return stats.CurrentDifficulty
	}
	targetSharesPerSec := 1.0 / float64(m.cfg.TargetSeconds)

	if stats.SharesPerSecond == 0 {
		// No accepted shares in the window: only ease difficulty after
		// the miner has been quiet for longer than two target intervals.
		if time.Since(stats.LastShareTime) > time.Duration(m.cfg.TargetSeconds*2)*time.Second {
			return stats.CurrentDifficulty * 0.9
		}
		return stats.CurrentDifficulty
	}

	// Adjust difficulty to reach the target share interval
	if stats.SharesPerSecond > targetSharesPerSec*1.2 {
		return stats.CurrentDifficulty * 1.2
	}
	if stats.SharesPerSecond < targetSharesPerSec*0.8 {
