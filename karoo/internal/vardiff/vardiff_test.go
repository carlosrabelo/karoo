...
func (m *mockClient) SetDiff(d int64) {
	m.diff = d
}
...
func TestCalculateNewDifficultyTargetRate(t *testing.T) {
	cfg := &Config{
		Enabled:       true,
		TargetSeconds: 10, // target 0.1 shares/sec
		MinDiff:       100,
		MaxDiff:       100000,
		AdjustEveryMs: 60000,
	}
	...
	stats := &ClientStats{
		...
		SharesPerSecond:   0.2, // twice as fast as target
	}

	got := mgr.calculateNewDifficulty(stats)
	if got <= stats.CurrentDifficulty {
		t.Fatalf("expected difficulty increase when too fast, got %v", got)
	}

	stats.SharesPerSecond = 0.05 // slower than target
	got = mgr.calculateNewDifficulty(stats)
	if got >= 1000 {
		t.Fatalf("expected difficulty decrease when too slow, got %v", got)
	}
	...
	got = mgr.calculateNewDifficulty(stats)
	if got != 1000 {
		t.Fatalf("expected difficulty unchanged with recent silence, got %v", got)
	}
}
