...
func TestNewManager(t *testing.T) {
	...
	if atomic.LoadUint32(&m.upReady) == 1 {
...
func TestEnqueuePendingSubscribe(t *testing.T) {
	...
	if string(m.pendingSubs[cl]) != string(id) {
		t.Errorf("Expected ID %s, got %s", id, m.pendingSubs[cl])
	}
	m.subMu.Unlock()
}
...
func TestFlushPendingSubscribes(t *testing.T) {
	...
	id1 := stratum.NewIntID(123)
	id2 := stratum.NewIntID(456)
...
func TestSetUpstreamReady(t *testing.T) {
	...
	// Test setting ready
	m.SetUpstreamReady(true)
	if atomic.LoadUint32(&m.upReady) == 0 {
...
func TestReset(t *testing.T) {
	...
	// Prefix counter is preserved across upstream reconnects to avoid
	// reissuing prefixes already held by live clients.
	_ = atomic.LoadUint64(&m.prefixCounter)
