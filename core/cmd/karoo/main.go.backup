...
func main() {
	...
	// Parse command line arguments manually for --option format
	args := os.Args[1:]
	for i := 0; i < len(args); i++ {
		...
		case "--listen":
			if i+1 < len(args) {
				cfg.Proxy.Listen = args[i+1]
				i++
			}
		case "--url":
			if i+1 < len(args) {
				parseURL(args[i+1], &cfg)
				i++
			}
		case "--user":
			if i+1 < len(args) {
				cfg.Upstream.User = args[i+1]
				i++
			}
		case "--pass":
			if i+1 < len(args) {
	...
	// Load config file if specified or try default
	if cfgFile == "" {
		cfgFile = "config.json"
	}

	if fileCfg, err := loadConfig(cfgFile); err == nil {
		// Merge: command line takes precedence
		if cfg.Proxy.Listen == "" {
			cfg.Proxy.Listen = fileCfg.Proxy.Listen
		}
		if cfg.Upstream.Host == "" {
			cfg.Upstream.Host = fileCfg.Upstream.Host
		}
		if cfg.Upstream.Port == 0 {
			cfg.Upstream.Port = fileCfg.Upstream.Port
		}
		if cfg.Upstream.User == "" {
			cfg.Upstream.User = fileCfg.Upstream.User
		}
		if cfg.Upstream.Pass == "" {
			cfg.Upstream.Pass = fileCfg.Upstream.Pass
		}
		if !cfg.Compat.StrictBroadcast {
			cfg.Compat.StrictBroadcast = fileCfg.Compat.StrictBroadcast
		}
		// Copy other defaults from file
		cfg.Proxy.ClientIdleMs = fileCfg.Proxy.ClientIdleMs
		cfg.Proxy.MaxClients = fileCfg.Proxy.MaxClients
		cfg.Proxy.ReadBuf = fileCfg.Proxy.ReadBuf
		cfg.Proxy.WriteBuf = fileCfg.Proxy.WriteBuf
		cfg.Upstream.TLS = fileCfg.Upstream.TLS
		cfg.Upstream.InsecureSkipVerify = fileCfg.Upstream.InsecureSkipVerify
		cfg.Upstream.BackoffMinMs = fileCfg.Upstream.BackoffMinMs
		cfg.Upstream.BackoffMaxMs = fileCfg.Upstream.BackoffMaxMs
	...
	go px.upstreamManager(ctx, 15*time.Second)
...
func parseURL(url string, cfg *Config) {
...
func printUsage() {
	...
	fmt.Printf("Options:\n")
	fmt.Printf("  --config <file>     Config file path (default: config.json)\n")
	fmt.Printf("  --listen <addr>     Proxy listen address (default: :3334)\n")
	...
	fmt.Printf("Examples:\n")
	fmt.Printf("  karoo --listen :3334 --url stratum+tcp://pool.example.com:3333 --user wallet.worker --pass x\n")
	fmt.Printf("  karoo --config myconfig.json --user wallet.worker2\n")
}
